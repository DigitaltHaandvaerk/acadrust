...
fn main() {
    ...
    for &(version, ver_str) in VERSIONS {
        ...
        // ── MultiLeader (R2007+ / class-based) ──────────────────────
        // MLEADER did not exist before AutoCAD 2008 (AC1021); emitting it
        // in older-format files audits clean in acadrust's own round-trip
        // but is anachronistic and gets "repaired" by real AutoCAD.
        ...
        if version >= DxfVersion::AC1021 {
            ...
            println!("  SKIP {:<20} not supported before R2007", "MULTILEADER");
            skip += 1;
        }
        ...
        gen(
            ...
            &mut skip,
            || {
                let sat_doc = build_cylinder([0.0, 0.0, 0.0], 5.0, 10.0);
    ...
    F: FnOnce() -> EntityType,
{
    ...
    match &mut entity {
        ...
        EntityType::Shape(shape) => {
            let mut style = TextStyle::new("ACADRUST_SHAPES");
            style.handle = doc.allocate_handle();
            style.is_shape_file = true;
            style.font_file = "ltypeshp.shx".to_string();
            let style_handle = style.handle;
            doc.text_styles.add(style).unwrap();
            shape.style_name = "ACADRUST_SHAPES".to_string();
            shape.style_handle = Some(style_handle);
            shape.shape_name.clear();
            shape.shape_number = 1;
        }
        EntityType::MultiLeader(mleader) => {
            mleader.style_handle = doc.objects.values().find_map(|object| {
                if let ObjectType::MultiLeaderStyle(style) = object {
                    (style.name == "Standard").then_some(style.handle)
                } else {
                    None
                }
            });
            mleader.text_style_handle = Some(doc.header.current_text_style_handle);
            mleader.context.text_style_handle = Some(doc.header.current_text_style_handle);
        }
        _ => {}
    }
